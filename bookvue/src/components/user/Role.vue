    ...
    <el-dialog
      ...
      <el-form v-model="selectedRole" style="text-align: left" ref="dataForm">
        ...
        <el-form-item label="角色描述" label-width="120px" prop="name">
          <el-input v-model="selectedRole.zhname" autocomplete="off"></el-input>
        </el-form-item>
    ...
    export default {
      ...
      methods: {
          ...
          if (role.id !== 1) {
            this.$confirm('是否更改角色状态？', '提示', {
              ...
              this.$axios.put('/role/status', {
                ...
                if (resp && resp.status === 200) {
                  if (value) {
                    this.$message('角色 [' + role.zhname + '] 已启用')
                  } else {
                    this.$message('角色 [' + role.zhname + '] 已禁用')
                  }
                }
              })
            }).catch(() => {
        ...
        onSubmit (role) {
          ...
          this.$axios.put('/role/menu?rid=' + role.id, {
